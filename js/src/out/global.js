...
var createProportionAreas = function(comparisons, actualProfit, actualTaxBreak, convertConst, name) {
  var money35 = actualProfit * 0.35 / convertConst,
  ...
  var filled = 0,
      lastComparison = 0;
  for (var ii in comparisons) {
    var comparison$__1 = comparisons[ii],
        comparisonMoney = comparison$__1['money'] / convertConst,
        numComparisonSquares = Math.floor(comparisonMoney);
    if (isWholeComparison(comparison$__1, numTaxBreakSquares, numComparisonSquares, filled)) {
      proportionAreas.push({
        'text': comparison$__1['text'],
        'numSquares': numComparisonSquares,
        'color': comparison$__1['color'],
  ...
  var squaresLeft = numTaxBreakSquares - filled,
  ...
  while (squaresLeft > 0 && nextComparisonIndex < comparisons.length) {
    var comparisonMoney$__2 = comparison['money'] / convertConst,
        multiple = Math.floor(squaresLeft / comparisonMoney$__2),
        numSquares = Math.ceil(comparisonMoney$__2 * multiple);
    if (multiple > 0) {
      proportionAreas.push({
        'text': multiple + ' x ' + comparison['text'],
        'numSquares': numSquares,
        'color': comparison['color'],
        'money': getMoneyString(comparisonMoney$__2 * multiple, convertConst)
      });
...
var getMoneyString = function(money, convertConst) {
  var unit,
      ordMag;
  money *= convertConst;
...
var isWholeComparison = function(comparison, numTaxBreakSquares, numComparisonSquares, filled) {
  var enough = numComparisonSquares > 4,
...
d3.queue().defer(d3.csv, 'csv/dv_data/interactive_data.csv', typeCompanies).defer(d3.csv, 'csv/dv_data/comparison_data.csv', typeComparisons).await(function(error, companies, comparisons) {
  ...
  companies.map(function(d) {
    ...
    var convertConst = Math.pow(10, (Math.floor(Math.log10(d['profit'])) - 3));
    if (convertConst < 1)
      convertConst = 1;
    comparisonData[d['company_name']] = createProportionAreas(comparisons, d['profit'], d['tax_break'], convertConst, d['company_name']);
    totalProfits += d['profit'];
    totalTaxBreaks += d['tax_break'];
  });
  var totalRate = ((totalProfits * 0.35 - totalTaxBreaks) / totalProfits * 100).toFixed(2);
  ...
  createSlides(companies, companiesYearsNoTax, companiesTop25, companiesRebates, companiesIPS, companiesTop3EmpChanges, companiesLostEmployees, companiesCompUp, companiesForeignDiff, companiesCompetitors);
});
