var loadInfo = function(infoBoxData) {
  $('.info').animate({'opacity': 0}, 500, function() {
    ...
    var taxBreak = infoBoxData['taxBreak'];
    if (taxBreak[0] == '-') {
...
var appendStoryText = function(duration, newText) {
  var clearDuration = arguments[2] !== (void 0) ? arguments[2] : false;
  var imgSrc = arguments[3] !== (void 0) ? arguments[3] : null;
  var isHuge = arguments[4] !== (void 0) ? arguments[4] : false;
  return new Promise(function(resolve, reject) {
    var chain = Promise.resolve();
    if (clearDuration != false) {
      chain = chain.then(function() {
        return new Promise(function(resolve, reject) {
          d3.select('.dynamic-text').selectAll('text').transition().duration(clearDuration).style('opacity', 0).remove().end(resolve);
        });
      });
    }
    chain.then(function() {
      d3.select('.dynamic-text').style('line-height', function() {
        return isHuge ? '40px' : '80px';
      }).append('text').style('opacity', 0).html(function() {
        if (imgSrc)
          newText = '<img src=' + imgSrc + ' class="dynamic-text-img"/>' + newText;
        return newText;
      }).transition().duration(duration).style('opacity', 1).end(resolve);
    });
  });
};
var updateStoryText = function(duration, newText, imgSrc) {
  return new Promise(function(resolve, reject) {
    d3.select('.dynamic-text').style('opacity', 0).transition().duration(duration).style('opacity', 1).call(function(d) {
      if (imgSrc)
        newText = '<img src=' + imgSrc + ' class="dynamic-text-img"/>' + newText;
      d3.select('.dynamic-text').html(newText);
    }).end(resolve);
  });
};
